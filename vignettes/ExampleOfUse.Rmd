...
regionSetFiles=(File Nathan gives me + MSC specific set)
BSDTList=lapply(X = RRBSFiles,FUN = BSreadBiSeq)
regionSets=fread(regionSetFiles)
regionSets=lapply(X = regionSets,FUN = setNames(c("chr","start","stop")))
```

Next let's divide each region into bins, find methylation that is contained in each bin in each region, and aggregate matching bins over all the regions.


```{r,eval=FALSE}
bigBin=lapply(X = BSDTList,FUN = returnMIRABins,GRList=regionSets,binNum=11,sampleNameInBSDT=TRUE)
...
merge(bigBinDT,sampleName,all.x=TRUE)#adding annotation
...
merge(sampleScores,sampleName,all.x=TRUE)#adding annotation

...
2. Choose parameters based on preliminary results:
    2.1. Pick a region size for each region set based on what the MIRA signatures look like.
    2.2. After binning again based on the new region sizes, you can pick what to count as outer edges of the dip for MIRA scoring (shoulderShift variable, discussed later).
3. Run the full MIRA analysis with chosen parameters.
...
The below code shows step 3 of the process (the step after choosing parameters that are appropriate for your data) although the same thing would be done for steps 1 and 2, just with fewer samples and different parameters. Choosing appropriate parameters is discussed more below.
